 ...
 * Filtre strict d'éligibilité pour offres Remote Afrique & Relocation/Visa
 */
...
export function filtrerParRegex(titre, description, locationRaw = '') {
  const texte = `${titre || ''}\n${locationRaw || ''}\n${description || ''}`;

  // 1. Vérification Visa Sponsorship / Relocation explicite positif
  ...
  // 2. Vérification des exclusions géographiques strictes
  ...
  // 3. Si la location_raw indique une région non-africaine (ex: US, Europe, Americas, LATAM) sans Worldwide explicite dans location_raw
  ...
  // 4. Si Worldwide / Africa explicite dans la description
  ...
  // 5. Autrement -> ambigu (pour vérification IA avec prompt ultra-strict)
