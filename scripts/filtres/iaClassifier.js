 ...
 * Classification ultra-stricte d'éligibilité géographique pour le Cameroun / l'Afrique.
 */
...
RÈGLE D'OR :
Une offre ne peut être retenue que sous 2 conditions strictement exclusives :
...
function classifierHeuristique(titre, entreprise, description, locationRaw = '') {
  const text = `${titre || ''}\n${entreprise || ''}\n${locationRaw || ''}\n${description || ''}`.toLowerCase();

  // Exclusions explicites de visa / relocation
  ...
  // Relocation & Visa Sponsorship détection
  ...
  // Remote Afrique / Worldwide détection
...
export async function classifierOffre(titre, entreprise, description, locationRaw = '') {
