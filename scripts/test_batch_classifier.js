/**
 * ESSOR Batch Classifier Test Script
 * Teste le pipeline en 2 étapes (Regex + IA) sur les offres actuelles de la base remote_jobs
 */
...
export async function testBatchClassification() {
  ...
  // 1. Récupération des offres existantes dans remote_jobs
  ...
  if (!res.ok) {
    console.error('❌ Impossible de récupérer les offres pour le batch test:', await res.text());
    return;
  }
  ...
  for (let i = 0; i < offers.length; i++) {
    ...
    console.log(`\n[${i + 1}/${offers.length}] "${offer.title}" (${offer.company}) - Source: ${offer.source}`);
    ...
    console.log(`   └─ Justification: ${classification.justification_remote || classification.justification_relocation}`);
    ...
    // Mise à jour de la ligne Supabase avec la classification
    const updatePayload = {
  ...
  console.log('\n====================================================');
  console.log('📊 RÉSULTAT DE LA CLASSIFICATION EN BATCH :');
  ...
  console.log('----------------------------------------------------');
  console.log(`   🟢 Offres Remote Afrique : ${countRemoteAfrique}`);
  console.log(`   ✈️ Offres Relocation / Visa Sponsorisé : ${countRelocation}`);
  console.log(`   🔴 Offres Exclues (Rejetées) : ${countAucun}`);
  console.log('====================================================');
}
