...
export const RegisterPage: React.FC = () => {
  ...
  const handleWhatsAppAuth = async () => {
    setErrorMessage(null);
    if (!phone.trim()) {
      setErrorMessage('Veuillez saisir votre numéro WhatsApp dans le champ ci-dessous.');
      setStep(1);
      return;
    }
    const result = await loginWithWhatsApp(phone);
    if (!result.success && result.error) {
      setErrorMessage(result.error);
    } else {
      alert('Un code de vérification a été envoyé sur votre WhatsApp !');
    }
  };
  ...
  return (
    <div className="pt-28 pb-20 px-4 sm:px-6 lg:px-8 max-w-4xl mx-auto">
      ...
      {emailSent ? (
        ...
        {/* Step indicator (Fully Clickable Tabs) */}
        <div className="flex items-center justify-between border-b border-sauge/30 pb-6 mb-8 gap-4">
          <button
            type="button"
            onClick={() => setStep(1)}
            className={`flex items-center gap-3 p-2.5 rounded-2xl transition-all cursor-pointer text-left ${
              step === 1 ? 'bg-sauge/30 border border-sauge' : 'hover:bg-sauge/15 opacity-70'
            }`}
          >
            <div className={`w-9 h-9 rounded-full font-sora font-bold text-sm flex items-center justify-center shrink-0 transition-all ${
              step === 1 ? 'bg-vert-profond text-creme shadow-md' : 'bg-sauge/40 text-vert-profond'
            }`}>
              1
            </div>
            <div>
              <p className="text-xs uppercase tracking-wider text-encre/60 font-bold">Étape 1 sur 2</p>
              <p className="font-sora font-bold text-sm text-vert-profond">Compte & WhatsApp</p>
            </div>
          </button>

          <div className="w-12 h-0.5 bg-sauge/40 hidden sm:block shrink-0"></div>

          <button
            type="button"
            onClick={() => setStep(2)}
            className={`flex items-center gap-3 p-2.5 rounded-2xl transition-all cursor-pointer text-left ${
              step === 2 ? 'bg-sauge/30 border border-sauge' : 'hover:bg-sauge/15 opacity-70'
            }`}
          >
            <div className={`w-9 h-9 rounded-full font-sora font-bold text-sm flex items-center justify-center shrink-0 transition-all ${
              step === 2 ? 'bg-vert-profond text-creme shadow-md' : 'bg-sauge/40 text-vert-profond'
            }`}>
              2
            </div>
            <div>
              <p className="text-xs uppercase tracking-wider text-encre/60 font-bold">Étape 2 sur 2</p>
        ...
        <div className="space-y-3 mb-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <button
              ...
              className="py-3 px-4 rounded-2xl border border-sauge/60 bg-white hover:bg-sauge/10 text-vert-profond font-sora font-bold text-xs transition-all shadow-subtle flex items-center justify-center gap-2.5"
            >
            ...
            <button
              ...
              className="py-3 px-4 rounded-2xl bg-whatsapp/15 border border-whatsapp/40 text-vert-profond hover:bg-whatsapp/25 font-sora font-bold text-xs transition-all shadow-subtle flex items-center justify-center gap-2.5"
            >
              <MessageSquare className="w-4 h-4 text-whatsapp fill-whatsapp" />
              <span>Connexion avec WhatsApp</span>
            </button>
          </div>
          ...
          <div className="relative py-2 flex items-center justify-center">
            ...
            <span className="bg-white px-3 text-[11px] font-bold text-encre/50 uppercase tracking-wider absolute">
              ou compléter manuellement par étape
            </span>
          </div>
        </div>
